...
if __name__ == '__main__':
    ...
    if mode == 'scanner':
      dataFolder = r'./Data/Phantom/Baseline'
      im_zy_folder = 'recon_img_corr_1_baseline_zy.npy'  # axial
      im_xy_folder = 'recon_img_corr_3_baseline_xy.npy'  # sagittal
      im_zx_folder = 'recon_img_corr_2_baseline_zx.npy'  # coronal
      ...
      im_axial = np.moveaxis(im_axial, [0, 1, 2], [2, 1, 0])  # zyx --> xyz
    ...
    make_nifti(data=im_axial, fname='axial_ph.nii.gz', mask=False,
               res=[2, 2, 2], dim_info=[2, 1, 0]) # phase, freq, slice
    axial_mask = im_axial > thresh
    # plot_anatomy_raw(axial_mask, clim=[0, 2])
    make_nifti(data=axial_mask, fname='axial_ph_mask.nii.gz', mask=True,
               res=[2, 2, 2], dim_info=[2, 1, 0])  # phase, freq, slice

    # Write sagittal data to nifti file
    make_nifti(data=im_sag, fname='sag_ph.nii.gz', mask=False,
               res=[2, 2, 2], dim_info=[0, 1, 2])  # phase, freq, slice
    sag_mask = im_sag > thresh
    # plot_anatomy_raw(sag_mask, clim=[0, 2])
    make_nifti(data=sag_mask, fname='sag_ph_mask.nii.gz', mask=True,
               res=[2, 2, 2], dim_info=[0, 1, 2])  # phase, freq, slice

    # Write coronal data to nifti file
    make_nifti(data=im_cor, fname='cor_ph.nii.gz', mask=False,
               res=[2, 2, 2], dim_info=[0, 2, 1])  # phase, freq, slice
    cor_mask = im_cor > thresh
    # plot_anatomy_raw(cor_mask, clim=[0, 2])
    make_nifti(data=cor_mask, fname='cor_ph_mask.nii.gz', mask=True,
               res=[2, 2, 2], dim_info=[0, 2, 1])  # phase, freq, slice

    debug = True
    if debug is True:

      im_sr = nib.load('srr_3cycle_19mm_Huber_ph.nii.gz')
