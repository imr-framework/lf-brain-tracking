...
if __name__ == '__main__':
    ...
    if mode == 'scanner':
      dataFolder = r'./Data/NHP'
      im_axial_nii = 'axial.nii.gz'  # axial
      im_sag_nii = 'sag.nii.gz'  # sagittal
      im_cor_nii = 'cor.nii.gz'  # coronal

      im_axial = nib.load(os.path.join(dataFolder, im_axial_nii)).get_fdata()
      im_sag = nib.load(os.path.join(dataFolder, im_sag_nii)).get_fdata()
      im_cor = nib.load(os.path.join(dataFolder, im_cor_nii)).get_fdata()
    ...
    make_nifti(data=im_axial, fname='./Data/NHP/axial_new.nii.gz', mask=False,
               res=[2, 2, 2], dim_info=[2, 1, 0]) # phase, freq, slice
    axial_mask = im_axial > thresh
    # plot_anatomy_raw(axial_mask, clim=[0, 2])
    make_nifti(data=axial_mask, fname='./Data/NHP/axial_mask.nii.gz', mask=True,
               res=[2, 2, 2], dim_info=[2, 1, 0])  # phase, freq, slice

    # Write sagittal data to nifti file
    make_nifti(data=im_sag, fname='./Data/NHP/sag_new.nii.gz', mask=False,
               res=[2, 2, 2], dim_info=[0, 1, 2])  # phase, freq, slice
    sag_mask = im_sag > thresh
    # plot_anatomy_raw(sag_mask, clim=[0, 2])
    make_nifti(data=sag_mask, fname='./Data/NHP/sag_mask.nii.gz', mask=True,
               res=[2, 2, 2], dim_info=[0, 1, 2])  # phase, freq, slice

    # Write coronal data to nifti file
    make_nifti(data=im_cor, fname='./Data/NHP/cor_new.nii.gz', mask=False,
               res=[2, 2, 2], dim_info=[0, 2, 1])  # phase, freq, slice
    cor_mask = im_cor > thresh
    # plot_anatomy_raw(cor_mask, clim=[0, 2])
    make_nifti(data=cor_mask, fname='./Data/NHP/cor_mask.nii.gz', mask=True,
               res=[2, 2, 2], dim_info=[0, 2, 1])  # phase, freq, slice

    debug = False
    if debug is True:

      im_sr = nib.load('srr_7.nii.gz')
