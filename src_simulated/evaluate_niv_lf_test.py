...
def evaluate_model(folder_path, model_name, X_test, y_test,
    ...
    print("\n✅ Evaluation complete.")
    return results, pred1, pred2, model1, model2
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------
    # Pad LF volume to match padded y_test
    # ------------------------------------------------------------
    # Pad LF volume to match y_test shape (batch, H, W, D)
    target_shape = y_test.shape[1:]  # (H, W, D)
    if im.shape != target_shape:
        pad_h = target_shape[0] - im.shape[0]
        pad_w = target_shape[1] - im.shape[1]
        pad_d = target_shape[2] - im.shape[2]
        ...
        pad_info_im = pad_width
    else:
        pad_info_im = [(0, 0), (0, 0), (0, 0)]
    print(f"Padded LF volume shape: {im.shape}")

    # ------------------------------------------------------------
    # Visualize mid-slices
    # ------------------------------------------------------------
    target_shape = y_test[0].shape  # (H, W, D)
    midz = target_shape[2] // 2
    plt.figure(figsize=(10, 5))
    plt.subplot(1, 2, 1)
    plt.imshow(im[:, :, midz], cmap='gray')
    plt.title("Resampled + Padded LF")
    plt.axis("off")
    ...
    # Optional: unpad prediction if evaluate_model returns output volume
    # pred_unpadded = unpad_volume(pred_volume, pad_info_im)
