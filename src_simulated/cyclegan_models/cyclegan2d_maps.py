...
def save_models(step, g_model_AtoB, g_model_BtoA, output_path='src_simulated/outputs/maps'):
...
def summarize_performance(step, g_model, trainX, name, n_samples=5, output_path='src_simulated/outputs/maps'):
...
def train(d_model_A, d_model_B, g_model_AtoB, g_model_BtoA,
    ...
    for i in range(n_steps):
        ...
        if (i + 1) % (bat_per_epo * 50) == 0:
            save_models(i, g_model_AtoB, g_model_BtoA,
                        output_path='src_simulated/outputs/maps')
...
# Paths
path = "src_simulated/maps/"
trainA_path = os.path.join(path, "trainA")
trainB_path = os.path.join(path, "trainB")

# Load data
dataA = load_images_from_folder(trainA_path, image_size=(256, 256))
dataB = load_images_from_folder(trainB_path, image_size=(256, 256))
