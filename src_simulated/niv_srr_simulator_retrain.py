...
test_days = [3, 4, 5]
output_path = "Output"
checkpoint_path = os.path.join(output_path, "residual_srr_unet_checkpoint1.keras")
refined_model_name = "residual_srr_unet_refined"
#residual_srr_unet_patch_checkpoint_checkpoint -- for patch
os.makedirs(output_path, exist_ok=True)

batch_size = 32
patch_xy, patch_z = 0, 0
steps_per_epoch = 24
...
lf_train_refined = predict_refined_inputs(base_model, X_train, patch_size=(144,144,40), overlap=0.0)
lf_val_refined = predict_refined_inputs(base_model, X_val, patch_size=(144,144,40), overlap=0.0)
...
# -----------------------------
refined_checkpoint_path = os.path.join(output_path, f"{refined_model_name}_checkpoint1.keras")
...
final_refined_path = os.path.join(output_path, f"{refined_model_name}_final1.keras")
