...
def load_nii_volumes(path, current_spacing=(1,1,2), target_spacing=(1,1,1)):
    """
    Load NIfTI volumes, resample by voxel spacing only,
    ensure H=W=128 or discard, crop/pad D→35, return list.
    """
    ...
    for fname in os.listdir(path):
        ...
        # ---------------------------------------
        # 1. RESAMPLING USING VOXEL SPACING ONLY
        # ---------------------------------------
        zoom_factors = (
        ...
        # ---------------------------------------
        # 2. DISCARD IF HEIGHT/WIDTH NOT 128×128
        # ---------------------------------------
        if h != TARGET_H or w != TARGET_W:
            print(f"Skipping volume {fname} — size mismatch:", vol_iso.shape)
            continue

        # ---------------------------------------
        # 3. CROP/PAD DEPTH to 35 (NO DISTORTION)
        # ---------------------------------------
        ...
        # ---------------------------------------
        # vol_fixed = vol_fixed[..., None]  # (128,128,35,1)

        print(f"Loaded {fname}: {vol_fixed.shape}")
        volumes.append(vol_fixed)

	# Convert safely to numpy array
    volumes = np.stack(volumes, axis=0)  # (N,128,128,35)
