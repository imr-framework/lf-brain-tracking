...
if __name__=='__main__':
    ...
    nii_name = './Data/MW/IRF_NHP/Selected_to_process/ToDo/59228_D22/nhp_59228_D_22_preprocessed_slices.nii.gz'
    nhp_num = '59228_D_22'
    ...
    new_dimz = int(np.ceil(low_res_im.shape[2] * recon_conf2.scale_factors[0][1]))
    ...
    if reuse_mask is False:
        mask_store  = np.zeros((low_res_im.shape),dtype=bool)
        for slice in range(slices):
            fig = plt.figure(1)
            print(Fore.GREEN + "Debug mode: display now")
            im_slice_norm = low_res_im[:, :, slice]
            plt.imshow(im_slice_norm, cmap='gray')
            r = RoiPoly(color='r', fig = fig)
            mask = r.get_mask(im_slice_norm)
            mask_store[:, :, slice] = mask
            im_slice_masked = np.multiply(im_slice_norm, mask)
            plt.imshow(im_slice_masked, cmap='gray')
            plt.show()
            low_res_im[:, :, slice] = im_slice_masked   
        np.save('mask_nhp_invivo.npy', mask_store)
    else:
        # mask_store = np.load('mask_nhp_invivo.npy')
        # low_res_im = np.multiply(low_res_im, mask_store)
        print('Not masking now. Will do post')
    ...
    for iter_srr in range(num_iters_srr):
        ...
        new_dimx = int(np.ceil(low_res_im.shape[0] * recon_conf.scale_factors[0][0]))
        new_dimy = int(np.ceil(low_res_im.shape[1] * recon_conf.scale_factors[0][1]))
        high_res_im = np.zeros((new_dimx, new_dimy,low_res_im.shape[2]))
        if contrast_enhance is True:
            high_res_im_ce = do_contrast_enhance(high_res_im)
            high_res_im = high_res_im_ce  
        if write_nifti is True:
            make_nifti(data=low_res_im * 2048, fname= fname_file_save + 'ZSSR_2D_ms_srr_1dot2x'+ str(iter_srr) +'.nii.gz', mask=False,
                res=[1, 1, 5], dim_info=[2, 1, 0]) # phase, freq, slice    

        if debug_mode is True:
            img = nib.load(fname_file_save + 'ZSSR_2D_ms_srr_1dot2x'+ str(iter_srr) +'.nii.gz')
