...
class ZSSR:
    ...
    def __init__(self, input_img, conf=configs.Config(), ground_truth=None, kernels=None):
        # Acquire meta parameters configuration from configuration class as a class variable
        self.conf = conf
        self.conf.run_test_every = 64
    ...
    def run(self):
        ...
        for self.sf_ind, (sf, self.kernel) in enumerate(zip(self.conf.scale_factors, self.kernels)):
            ...
            if self.conf.save_results:
                ...
                plt.imsave('%s/zssr_%s.png' %
                           (self.conf.result_path, sf_str),
    ...
    def base_change(self):
        ...
        if (abs(np.asarray(self.conf.scale_factors[self.sf_ind]) - np.asarray(self.conf.base_change_sfs[self.base_ind])) < 0.001).all():
            ...
            print('base changed to: ', self.base_sf)
